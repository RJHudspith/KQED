		     ...
		     struct QED_Kernels *K )
{
  ...
  for( i = 0 ; i < 384 ; i++ ) {
    ...
    register const double Lx0mulam = *(pLx0ml + idx) ;
    register const double Ly0mulam = *(pLy0ml + idx) ;
    register const double f1 = ( Lx0mulam + Ly0mulam )/2. ;
    register const double f2 = ( *pLx0 + *pL0y ) ;
    register const double f4 = ( *pL0x + *pLy0 ) ;
    register const double f3 = ( *pL0x - *pL0y ) ;
